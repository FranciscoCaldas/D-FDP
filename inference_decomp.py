...
from utils.util import find_max_epoch, print_size, sampling_p2, calc_diffusion_hyperparams,calc_diffusion_hyperparams2,calc_quantile_CRPS,calc_sample_CRPS
...
from dtl_utils.datasets_torchloaders import ETTm1TestDataset, ElectricityTestDataset, PTBXLTestDataset,Synth1TestDataset,SolarEnergyTestDataset,TempRainTestDataset,MujocoTestDataset

import warnings
warnings.filterwarnings("ignore", message=".*pyKeOps.*deprecated.*dtype.*")
...
def generate(output_directory,
    ...
    if trainset_config['train_data_path'] is not None:    
        if trainset_config['test_data_path'] == "./datasets/Mujoco/test_mujoco.npy":
            ...
            mujoco_test_dataset = MujocoTestDataset(trainset_config)
            testing_data = DataLoader(mujoco_test_dataset, batch_size=trainset_config['batch_size'], shuffle=False)
            print('Data loaded')
            ...
            testing_data = DataLoader(ettm1_test_dataset, batch_size=trainset_config['batch_size'], shuffle=False)
            print('Data loaded')
        ...
        elif trainset_config['test_data_path'] == "./datasets/solar_energy/solar_test_scaled.npy":
            dataset = SolarEnergyTestDataset(trainset_config)
            testing_data = DataLoader(dataset, batch_size=1000, shuffle=False)
            print('Data loaded')
        elif trainset_config['test_data_path'] == "./datasets/Temp_rain/Temp_rain_test_scaled.npy":
            dataset = TempRainTestDataset(trainset_config)
            testing_data = DataLoader(dataset, batch_size=1000, shuffle=False)
            print('Data loaded')
        else:
    ...
    # Peek at first batch to get batch size for validation
    expected_batch_size = None
    for peek_batch in testing_data:
        expected_batch_size = peek_batch.size(0)
        break

    # Check for existing output files and determine which batches to process
    completed_batches = set()
    if os.path.isdir(ckpt_path):
        for filename in os.listdir(ckpt_path):
            if filename.startswith('imputation') and filename.endswith('.npy'):
                try:
                    batch_idx = int(filename.replace('imputation', '').replace('.npy', ''))
                    # Verify the batch has all three required files
                    imputation_file = os.path.join(ckpt_path, f'imputation{batch_idx}.npy')
                    mask_file = os.path.join(ckpt_path, f'mask{batch_idx}.npy')
                    original_file = os.path.join(ckpt_path, f'original{batch_idx}.npy')
                    
                    if os.path.exists(mask_file) and os.path.exists(original_file):
                        imputation_size = os.path.getsize(imputation_file)
                        mask_size = os.path.getsize(mask_file)
                        original_size = os.path.getsize(original_file)
                        
                        # Only mark as completed if all files have reasonable sizes (> 100 bytes)
                        if imputation_size > 100 and mask_size > 100 and original_size > 100:
                            # Verify batch size matches by loading and checking shape
                            try:
                                original_data = np.load(original_file)
                                saved_batch_size = original_data.shape[0]
                                if saved_batch_size == expected_batch_size:
                                    completed_batches.add(batch_idx)
                                    print(f"Batch {batch_idx} already processed (batch size: {saved_batch_size}). Skipping.")
                                else:
                                    print(f"Batch {batch_idx} has mismatched batch size. Expected {expected_batch_size}, got {saved_batch_size}. Will reprocess.")
                            except Exception as e:
                                print(f"Could not load batch {batch_idx} for size verification: {e}. Will reprocess.")
                except (ValueError, OSError):
                    pass
    
    if completed_batches:
        print(f"Found {len(completed_batches)} already completed batches: {sorted(completed_batches)}")
    else:
        print("No completed batches found. Processing all batches.")
    ...
    with torch.no_grad():
        ...
        for i,batch in enumerate(testing_data):
            ...
            if i in completed_batches:
                print(f"Batch {i} already completed. Skipping inference.")
                continue
            ...
            #ablation study
            if sampling_with_dk == 2:
                
                print('using dk from data ablation')
                mask_2,loss_mask_2 = get_mask(batch, masking, 999)
                batch = batch.permute(0,2,1)
                dk = util.masked_components_fft_amplitude(batch, mask_2,max_components,masking,fixed=fixed_arguments,decomposition_method=decomposition_method)
            else:
                batch = batch.permute(0,2,1)  # Define mask generation separately
                if max_components > 0:
                    dk = util.masked_components_fft_amplitude(batch, mask,max_components,masking,fixed=fixed_arguments,decomposition_method=decomposition_method)
                else:
                    dk = None
            ...
            for j in range(num_obs):
                ...
                generated_audio,_  = sampling_p2(net, (batch_len, sample_channels, sample_length),
            ...
            print(f"Batch {i} MSE: {mse:.6f}")
            print(f"Batch {i} MAPE: {mape:.6f}")
            print(f"Batch {i} MAE: {mae:.6f}")
            print(f"Batch {i} MAPE2: {mape2:.6f}")
            print(f"Batch {i} CRPS: {crps:.6f}")
            print(f"Batch {i} CRPS sample: {crps_sample:.6f}")
    ...
    with open(output_file_path, 'a') as f:
        f.write(f"Results for checkpoint iteration: {ckpt_iter}\n")
        f.write(f"type of dk used: {sampling_with_dk}\n")
...
if __name__ == "__main__":
    ...
    generate(**gen_config,
             ...
             decomposition_method=train_config["decomposition_method"]
             )
