...
def forward_process_non_monte_carlo(y, components, diffusion_hyperparams,target_snr=1):
    ...
    #print(f"  NaN count components_sorted, forward3: {torch.isnan(y).sum()}",flush=True)
    #print(f"  NaN count components_sorted, forward3: {torch.isnan(components).sum()}",flush=True)
    ...
    #print(f"  NaN count components_sorted, forward3: {torch.isnan(sig_var).sum()}",flush=True)
    ...
    #noise_var[:,:,:] = 1- noise_var[:,:,:-1].sum(dim=-1)debug
    #nan_mask = torch.isnan(noise_var)
    #print(noise_var[63,6, :],'badbadbad')
    #print(noise_var[62,:, :],'example_noise_var_before_division')
    #if nan_mask.any():
    #    nan_indices = torch.where(nan_mask)
    #    print(f"  NaN indices BEFORE division: {list(zip(*[idx.tolist() for idx in nan_indices]))[:10]}", flush=True)
    #    print(noise_var[nan_mask])

    #sum_vals = noise_var[:,:,:].sum(dim=-1, keepdim=True)
    #print(f"  Sum values - Min: {sum_vals.min()}, Max: {sum_vals.max()}", flush=True)
    #print(f"  Sum has NaN: {torch.isnan(sum_vals).any()}", flush=True)
    #print(f"  Sum has zero: {(sum_vals == 0).any()}", flush=True)
    #print(f"  Sum has Inf: {torch.isinf(sum_vals).any()}", flush=True)

    #zero_mask = (sum_vals == 0)
    #if zero_mask.any():
    #    zero_indices = torch.where(zero_mask)
    #    print(f"  Division by ZERO at indices: {list(zip(*[idx.tolist() for idx in zero_indices]))[:10]}", flush=True)
    #    print(sum_vals[zero_mask])

    noise_var[:,:,:].clamp_min_(1e-5)
    #print(f"  NaN count noise_var, forward33: {torch.isnan(noise_var).sum()}",flush=True)
    noise_var[:,:,:] = noise_var[:,:,:]/ noise_var[:,:,:].sum(dim=-1,keepdim=True)
    #print(f"  NaN count noise_var, forward44: {torch.isnan(noise_var).sum()}",flush=True)
    #print(noise_var[63,6, :],'badbadbad')
    ...
    z_k_l = z_k_l + epsilon
    y_total_out = z_k_l
    #print(f"  NaN count scaled_variance, forward4: {torch.isnan(scaled_variance).sum()}",flush=True)
    #print(f"  NaN count components_sorted, forward4: {torch.isnan(z_k_l).sum()}",flush=True)
    #print(f"  NaN count components_sorted, forward4: {torch.isnan(epsilon).sum()}",flush=True)
...
def masked_components_fft_amplitude(y,mask,max_components,masking = 'forecast',fixed = True,order='desc',decomposition_method='fft'):
    '''
...
def sampling_p(net, size, diffusion_hyperparams, cond, mask,max_components,dk = None, only_generate_missing=1, 
...
def sampling_p_(net, size, diffusion_hyperparams, cond, mask,max_components,dk = None, only_generate_missing=1, guidance_weight=1,max_components_gen=7,sampling_with_dk = 1,dynamic = True):
    """
    Perform the complete sampling step according to p(x_0|x_T) = \prod_{t=1}^T p_{\theta}(x_{t-1}|x_t)
...
def training_loss(net, loss_fn, X, diffusion_hyperparams, only_generate_missing=1):
    ...
    if only_generate_missing == 1:
        ...
        z = z * (1 - mask).float() #author CHANGE (correct version, audio was being added twice)
...
def apply_forward(signal, diffusion_hyperparams, diffusion_steps, components_steps, target_snr=1, max_components = None ,monte_carlo=True,decomposition_method='fft'):
    ...
    #print(f"  NaN count components, forward1: {torch.isnan(components).sum()}",flush=True)

    if max_components == 0:
        #print("Warning: Max_components is 0, normal diffusion process will be used.")
        ...
        return y_total_out, y_total_in, y_total_noise , _dh ,torch.ones(signal.shape[0],signal.shape[1]).cuda()
    if monte_carlo == True:
    ...
    if monte_carlo:
        ...
        #print(f"  NaN count components_sorted, forward2: {torch.isnan(components_sorted).sum()}",flush=True)

        y_total_out,y_total_noise,dk_scaled , _dh= forward_process_non_monte_carlo(y_total, components_sorted, _dh,target_snr)

        #print(f"  NaN count components_sorted, forward2: {torch.isnan(y_total_out).sum()}",flush=True)
        #print(f"  NaN count components_sorted, forward2: {torch.isnan(y_total_noise).sum()}",flush=True)
        #print(f"  NaN count components_sorted, forward2: {torch.isnan(dk_scaled).sum()}",flush=True)
        #print(components_sorted.var(dim=3))
        #print(f"  NaN count components_sorted.var(dim=3), forward2: {torch.isnan(components_sorted.var(dim=3)).sum()}",flush=True)
...
def sampling_p2(net, size, diffusion_hyperparams, cond, mask,max_components,dk = None, only_generate_missing=1, guidance_weight=1,max_components_gen=7,sampling_with_dk = 1,dynamic = True):
    """
    Perform the complete sampling step according to p(x_0|x_T) = \prod_{t=1}^T p_{\theta}(x_{t-1}|x_t)
    ...
    if sampling_with_dk == 1:
        print('sampling with custom dk (for ablation)')
        dk = torch.ones_like(cond[:,:,:max_components_gen]).cuda() / max_components
    elif isinstance(sampling_with_dk, list):
        print('sampling with custom dk list (for ablation)')
        dk_list = torch.tensor(sampling_with_dk, dtype=cond.dtype, device=cond.device)
        dk_list = dk_list.view(1, 1, -1)
        dk = dk_list.repeat(cond.size(0), cond.size(1), 1)
    ...
    for n in range(N):
        ...
        x_all[:, :, n, :] = std * std_normal(cond.size())  # shape: (B, C, T) # i know this sounds silly, there is no need to do it like this, is just to be cristal clear
...
def synthetic_wavelet(signal, k,fixed=True, J=8, wave='db4'):
    ...
    top_k_indices = torch.topk(variances, 4,dim=0).indices
...
def training_loss_p(net, loss_fn, X, diffusion_hyperparams,max_components, only_generate_missing=1,monte_carlo=True,logging=None,decomposition_method='fft'):
    """
    Compute the training loss of epsilon and epsilon_theta
    ...
    #transformed_X = torch.sqrt(Alpha_bar[diffusion_steps]) * audio + torch.sqrt(
    #    1 - Alpha_bar[diffusion_steps]) * z  # compute x_t from q(x_t|x_0)
        transformed_X,Transformed_X_prior, transformed_X_noise , _dh, dk_scaled = apply_forward(audio,_dh, diffusion_steps, components_steps,max_components = max_components, monte_carlo=monte_carlo,decomposition_method=decomposition_method)
    # I want the output to be transformed_X 
    elif only_generate_missing == 1:
    
        transformed_X,transformed_X_prior, transformed_X_noise , _dh, dk_scaled = apply_forward(audio,_dh, diffusion_steps, components_steps,max_components = max_components, monte_carlo=monte_carlo,decomposition_method=decomposition_method)
        # Transformed X noise info
    ...
    #masked_output = epsilon_theta[loss_mask]
    #masked_target = transformed_X_noise[loss_mask]
    ...
    #legacy method currently only supports only_generate_missing = 1
    if only_generate_missing == 1:
        if isinstance(loss_fn, torch.nn.MSELoss):
            #if torch.isnan(loss_fn(epsilon_theta[loss_mask], transformed_X_noise[loss_mask])):
            #    print("Loss is NaN before backward")
            #print("Output:",epsilon_theta[loss_mask],flush=True)
            #print("Target:", transformed_X_noise[loss_mask],flush=True)
            #print("Difference:", epsilon_theta[loss_mask] - transformed_X_noise[loss_mask],flush=True)
            return loss_fn(epsilon_theta[loss_mask], transformed_X_noise[loss_mask])
        else:
...
def log_training_step(epoch, batch_idx, net, loss, optimizer, log_file="debug_log.txt"):
    """Log detailed training diagnostics"""
    ...
    # Clip gradients
    total_norm_after = torch.nn.utils.clip_grad_norm_(net.parameters(), max_norm=1.0)
    ...
    log_msg = (
        ...
        f"Loss: {loss.item():.6f} | "
        f"GradNorm: {total_norm_before:.6f} → {total_norm_after:.6f} | "
        f"AvgWeightNorm: {avg_weight_norm:.6f} | "
        f"LR: {current_lr:.2e} | "
        f"NaN/Inf: Loss({has_nan_loss}/{has_inf_loss}) Grad({grad_has_nan}/{grad_has_inf})"
    )
    ...
    if has_nan_loss or has_inf_loss:
        print("⚠️  WARNING: NaN or Inf detected in loss!")
    if grad_has_nan or grad_has_inf:
        print("⚠️  WARNING: NaN or Inf detected in gradients!")
    if total_norm_before > 100:
        print(f"⚠️  WARNING: Very large gradient norm ({total_norm_before:.2f})")
    
    return {
        'loss': loss.item(),
        'grad_norm_before': total_norm_before,
        'grad_norm_after': total_norm_after,
        'weight_norm': avg_weight_norm,
        'has_nan': has_nan_loss or grad_has_nan,
        'has_inf': has_inf_loss or grad_has_inf
    }
